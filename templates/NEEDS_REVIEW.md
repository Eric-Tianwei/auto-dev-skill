...
- 建议: 选 or-jwt-signing（p99=45ms，跨服务共享登录需求明确）；在 dag.json 里写 `or_groups[0].decided = "or-jwt-signing"`
- 状态: `or_groups[0].decided` 待人类填

## 2026-04-15 16:20 · or-jwt-refresh · node-stuck
- 现象: 重试 3 次均失败，失败信号都指向 middleware 未暴露 refresh 钩子
- 已尝试: 节点内换了 3 种 refresh 实现
- 建议: 改 dag.json 加新节点 `mw-refresh-hook` 在 `mw-validation` 之后、`or-jwt-refresh` 之前（改 deps 和 edges）；或改 `.auto-dev/nodes/or-jwt-refresh.md` 的 Completion 放宽对 refresh 的要求
